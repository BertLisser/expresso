...
public class RascalServer {
  ...
  public RascalServer(IValueFactory vf) {
    this.vf = vf;
  ...
  public void serve(ISourceLocation url, final IValue openCallback, final IValue msgCallback, final IValue closeCallbak, IBool asDeamon, final IEvaluatorContext ctx) {
    URI uri = url.getURI();
    ...
    if (asDeamon.getValue()) {
        ...
        executor = buildRegularExecutor((ICallableValue) openCallback);
    }
    else {
        mainThreadExecutor = new ArrayBlockingQueue<>(1024, true);
        executor = asyncExecutor((ICallableValue) openCallback, mainThreadExecutor);
    }
    ...
    NanoHTTPD server = new NanoHTTPD(host, port) {
      ...
      public Response serve(String uri, Method method, Map<String, String> headers, Map<String, String> parms,
        ...
        try {
          IValue request = makeRequest(uri, method, headers, parms, files);
